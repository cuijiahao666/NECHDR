...
def tensor_tonemap(x):
    return (torch.log(1 + 5000 * x) / torch.log(1 + 5000))
...
def perturb_low_expo_imgs(args, ldrs, expos, prob):
...
def perturb_low_expo_single_img(args, ldrs, expos, prob):
    
    if prob < 0.9:
        # cur_l_idx = torch.zeros(expos[0].shape, device=expos[0].device).byte()
        # exp1_list = [1] * len(expos)
        # cur_l_idx = cur_l_idx | (expos <= exp1_list).view(-1)

        for i in range(len(expos)):
            if expos[i] <= 1:                
                if args.tone_low:
                    ldrs[i] = pt_tone_ref_add_gaussian_noise(ldrs[i], stdv1=1e-4, stdv2=1e-3, scale=False)
                elif args.tone_ref:
                    ldrs[i] = pt_tone_ref_add_gaussian_noise(ldrs[i], stdv1=1e-3, stdv2=1e-3, scale=False)
                else:
                    raise Exception('Unknown tone low mode')
    # else:
    #     "--------------------------------加入复杂的噪声建模--------------------------------------------"
    ...
    #             for j in range(i, args.nframes, args.nexps): # e.g., [0,2], [0,3]
    #                 if args.tone_low:
    #                     ldrs[j][cur_l_idx] = degradation_bsrgan_plus_hdr(ldrs[j][cur_l_idx], shuffle_prob=0.1)
    #                 elif args.tone_ref:
    #                     ldrs[j][cur_l_idx] = degradation_bsrgan_plus_hdr(ldrs[j][cur_l_idx], shuffle_prob=0.1)
    #                 else:
    #                     raise Exception('Unknown tone low mode')