...
parser.add_argument("--dataset", type=str, default='CinematicVideo', choices=['DeepHDRVideo', 'CinematicVideo'],
                        help='dataset directory')
parser.add_argument("--dataset_dir", type=str, default='/data1/cuijiahao/HDRVIDEO/data/Synthetic_Dataset/HDR_Synthetic_Test_Dataset-001',
                        help='dataset directory')
parser.add_argument('--pretrained_model', type=str, default='./pretrained_models/2E/checkpoint_ours.pth')
parser.add_argument('--save_results', action='store_true', default=True)
parser.add_argument('--save_dir', type=str, default="./output_results/2E_ours")
...
def main():
    ...
    with torch.no_grad():
        for idx, img_data in enumerate(test_loader):
            ...
            start = time.time()
            #--------------------是否保存曝光补全帧结果和光流计算结果--------------------------
            #pred_hdr, pred_ip, flow_preds = model(pad_ldrs, gt_hdr, 0, expos, test_mode=True)
            pred_hdr = model(pad_ldrs, gt_hdr, 0, expos, test_mode=True)
            end = time.time()
            print('time',end-start)
            time_list.update(end-start)
            torch.cuda.synchronize()
            
            pred_hdr = padder.unpad(pred_hdr)
            pred_hdr = torch.squeeze(pred_hdr.detach().cpu()).numpy().astype(np.float32).transpose(1,2,0)
            #pred_ip = padder.unpad(pred_ip)
            #pred_ip = torch.squeeze(pred_ip.detach().cpu()).numpy().astype(np.float32).transpose(1,2,0)
            #save_flo(flow_preds, idx+1, args)

            #---------------------根据参考帧的Y通道曝光值，制作mask，保留参考帧曝光区域，并将曝光不好区域用预测的HDR结果代替----------------------
            ...
            if args.save_results:
                ...
                cv2.imwrite(os.path.join(hdr_output_dir, 'out_{:0>3d}.png'.format(idx+1)), (pred_hdr_tm*255.)[:,:,[2,1,0]].astype('uint8'))
                #cv2.imwrite(os.path.join(hdr_output_dir, 'ip_{:0>3d}_pred.png'.format(idx+1)), (pred_ip*255.)[:,:,[2,1,0]].astype('uint8'))
                #save_hdr(os.path.join(hdr_output_dir, 'out_{:0>3d}.hdr'.format(idx+1)), pred_hdr)
    print("infer_time:", time_list.avg)
    print("infer_time_list:", time_list)
