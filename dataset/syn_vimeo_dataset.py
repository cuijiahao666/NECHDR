...
class Syn_Vimeo_Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.nexps == 2:
            exposures, rev_exposures = self._get_2exposures(index)
        elif self.nexps == 3:
        ...
        hdrs, ldrs, rev_ldrs = self.re_expose_ldrs(hdrs, exposures, rev_exposures)
        # hdrs, rev_ldrs = self.re_expose_ldrs(hdrs, rev_exposures)
        ...
        for i in range(len(ldrs)):
            ...
            rev_ldr = rev_ldrs[i].astype(np.float32).transpose(2, 0, 1)
            rev_ldr_tensor = torch.from_numpy(rev_ldr)
            rev_ldrs_tensor.append(rev_ldr_tensor)
            ...
            rev_expos_tensor.append(torch.tensor(rev_exposures[i]))

        sample = {
            'hdrs': hdrs_tensor, 
            'ldrs': ldrs_tensor,
            'expos': expos_tensor,
            'rev_ldrs': rev_ldrs_tensor,
            'rev_expos': rev_expos_tensor
            }
        return sample
    ...
    def _get_2exposures(self, index):
        ...
        rev_exposures = np.ones(self.nframes, dtype=np.float32)
        ...
        if cur_high:
            ...
            for i in range(1, self.nframes, 2):
                rev_exposures[i] = high_expo
        else:
            for i in range(1, self.nframes, 2):
                exposures[i] = high_expo
            for i in range(0, self.nframes, 2):
                rev_exposures[i] = high_expo
        return exposures, rev_exposures
    ...
    def re_expose_ldrs(self, hdrs, exposures, rev_exposures):
        mid = len(hdrs) // 2
        new_hdrs = []
        if self.nexps == 3:
            if exposures[mid] == 1:
        ...
        for i in range(len(new_hdrs)):
            ...
            ldrs.append(ldr)
            rev_ldr = hdr_to_ldr(new_hdrs[i], rev_exposures[i])
            rev_ldrs.append(rev_ldr)

        return new_hdrs, ldrs, rev_ldrs
