...
def fetch_dataloader_2E(args):
    ...
    train_loader = DataLoader(train_vimeo, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers, pin_memory=True)
    val_vimeo = Syn_Test_Dataset(root_dir=args.dataset_test_dir, nframes=3, nexps=2)
...
def fetch_dataloader_3E(args):
    ...
    val_vimeo = Syn_Test_Dataset(root_dir=args.dataset_vimeo_dir, nframes=5, nexps=3, is_training=False)
    val_loader = DataLoader(val_vimeo, batch_size=args.val_batch_size, shuffle=False, num_workers=args.num_workers, pin_memory=True)   
    return train_loader, val_loader
